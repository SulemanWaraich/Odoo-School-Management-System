...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const userData = await register(email, password, name, role);
      toast.success('Registration successful!');
      const routes = { admin: '/admin', teacher: '/teacher', student: '/student' };
      navigate(routes[userData.role] || '/');
    } catch (error) {
      toast.error(formatApiErrorDetail(error.response?.data?.detail) || 'Registration failed');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <Card className="w-full max-w-md shadow-2xl border-0">
        <CardHeader className="text-center pb-2">
          ...
          <CardDescription>
            Join the school management portal
          </CardDescription>
        </CardHeader>
